...
np_cyclone <- str_c(cyclone_data_address, np_cyclone_filename, sep = "") |>
    ...
    mutate(
      ...
      across(max_wind:r_max_wind, as.numeric)
    )
...
convert_latlon <- function(latlon) {
    # YOUR CODE HERE
    parse_number(latlon) * if_else(str_detect(latlon, "[SW]"), -1, 1)
}
...
cyclones_data_update_2 <- cyclones_data_update_1 |>
    mutate(
        # YOUR CODE HERE
        date = make_datetime(year = ObservYear,
        month = Month,
        day = Day,
        hour = Hour,
        min = Minute)
...
covid_2020_match_country <- covid_2020 |>
  mutate(
    country = case_when(
      ...
      country == "Micronesia (country)" ~ "Micronesia, Fed. Sts.",
      ...
      country == "Laos" | country == "Lao People's Democratic Republic" ~ "Lao PDR",
      country == "North Macedonia" | country == "North MAcedonia" ~ "North Macedonia",
...
aware_pop <- inner_join(
  ...
  select(country, total_population,
         aware_no, aware_alittle, aware_moderate, aware_alot, aware_refuse)
aware_pop
```
